    ...
    <el-container style="flex: 1">
      ...
      <el-main>
        <div v-if="activeContent === 'open'" class="form-container">
          <div class="form-item">
            <el-button class="button1" @click="checkIdentity">检查用户身份</el-button>
          </div>
          <div class="form-item">
            <span>用户id:</span>
    ...
    .then((response) => {
      const dataWithRank = response.data.map((item, index) => {
        return {
          ...
          returnTime: dealWithTime(item.returnTime),
          managerId1: item.managerId1,
          managerId2: item.managerId2,
          id: item.id,
